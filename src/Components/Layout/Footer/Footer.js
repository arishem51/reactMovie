...
export default function Footer() {
    ...
    const arrRap =  _.map(heThongRap, item=> _.pick(item,['logo']));
    const renderLogo = ()=>{
        return arrRap.map((item,index)=>{
            return <li key={index}>
                    <img className='cursor-pointer my-1 w-14 h-14' src={item.logo} alt={index}></img>
            </li>
        })
    }
    return (
        <footer className="py-6 bg-coolGray-100 mt-10 text-coolGray-900 border-t-2 ">
