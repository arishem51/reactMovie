...
export default function Login(props) {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            taiKhoan: Yup.string().required("Tài khoản không được bỏ trống !!!").min(6, "Tài khoản ít nhất phải 6 ký tự").max(32, "Tài khoản tối đa là 32 ký tự"),
            matKhau: Yup.string().required("Mật khẩu không được bỏ trống !!!").min(6, "Mật khẩu ít nhất phải 6 ký tự").max(32, "Mật khẩu tối đa là 32 ký tự")
        }),
        onSubmit: values => {
        ...
        <form onSubmit={formik.handleSubmit} className="lg:w-1/2 xl:max-w-screen-sm">
            <div className="py-12 bg-indigo-100 lg:bg-white flex justify-center lg:justify-start lg:px-12">
                <div className="cursor-pointer flex items-center">
                    ...
                    <div className="text-2xl text-indigo-800 tracking-wide ml-2 font-semibold">Alien</div>
                </div>
            </div>
                ...
                <div className="mt-12">
                    <div>
                        <div>
                            ...
                            <input onBlur={formik.handleBlur} name='taiKhoan' onChange={formik.handleChange} className="w-full text-lg py-2 border-b border-gray-300 focus:outline-none focus:border-indigo-500" type placeholder="Nhập Tài Khoản" />
                            {formik.touched.taiKhoan && formik.errors.taiKhoan ? <p className='text-red-600 m-0 mt-2'>{formik.errors.taiKhoan} </p> : null}
                        ...
                        <div className="mt-8">
                            ...
                            {formik.touched.matKhau && formik.errors.matKhau ? <p className='text-red-600 m-0 mt-2'>{formik.errors.matKhau} </p> : null}
                        </div>
                        <div className="mt-10">
