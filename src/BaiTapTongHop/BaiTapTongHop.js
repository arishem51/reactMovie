...
export default function BaiTapTongHop() {
    ...
    return (
        <div>
        <PopUp popUp={popUp} setPopUp={setPopUp} itemModal ={itemModal}></PopUp>
        <div className='grid grid-cols-6'>
                    ...
                    <div className="divide-y divide-coolGray-300">
                        <ul className="pt-2 pb-4 space-y-1 text-sm">
                           {mangDanhMuc.map((danhMuc,index)=>{
                               const classActive = danhMuc.maDanhMuc === maDanhMuc ? [ "bg-coolGray-100", "text-coolGray-900", "active" , 'rounded-md'] : ["bg-coolGray-100", "text-coolGray-900" , "rounded-md"]
                               return  <li onClick={()=>{
                                   setMaDanhMuc(danhMuc.maDanhMuc)
                               }} className={_.join(classActive,' ')} key={index}>
            ...
            <div className='col-span-5'>
                <h3 className='text-blue-600 font-bold text-9xl mt-9'>Menu</h3>
                <section className="text-gray-600 body-font">
                    <div className="container px-5 py-24 mx-auto">
                        <div className="flex flex-wrap -m-4">
                           {_.filter(data,item=>item.maDanhMuc === maDanhMuc).map((doAn,index)=>{

                               return <div className="p-4 md:w-1/3" key={index}>
                                <div className="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
                                    <img className="lg:h-48 md:h-36 w-full object-cover object-center" src={doAn.hinhAnh} alt="blog" />
                                    <div className="p-6">
                                        <h2 className="tracking-widest text-xs title-font font-medium text-gray-400 mb-1">{doAn.tenDanhMuc}</h2>
                                        <h1 className="title-font text-lg font-medium text-gray-900 mb-3">{doAn.tenMonAn}</h1>
                                        <p className="leading-relaxed mb-3">Giá : {doAn.gia}$</p>
                                        <div className="flex items-center flex-wrap ">
                                            <a className="text-indigo-500 inline-flex items-center md:mb-2 lg:mb-0 cursor-pointer" onClick={()=>{
                                                setPopUp(true)
                                                setItemModal(doAn)
                                            }}>Chi Tiết
